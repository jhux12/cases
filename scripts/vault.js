    ...
    return `
      <div class="prize-card relative rounded-xl p-4 bg-white border-2 text-gray-900 text-center shadow-sm transition-transform duration-200 hover:scale-105" style="border-color:${color}">
        ...
        <div class="absolute bottom-2 left-2 flex items-center gap-1 text-yellow-600 font-medium text-xs">
          <img src="https://firebasestorage.googleapis.com/v0/b/hyperdrop-6476c.firebasestorage.app/o/packswebp%2Fripzacoin.webp?alt=media&token=e04bf3dc-a5b1-4b7e-89e1-125e144a3013" class="w-4 h-4" />
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (pfBtn) {
    ...
    const fairData = fairSnap.val();
      ...
      document.getElementById('pf-server-seed').textContent = fairData?.serverSeedHash || 'Hidden (server managed)';
    ...
    document.getElementById('new-server-seed').addEventListener('click', async () => {
      showToast('Server seeds rotate automatically after each spin.', 'bg-blue-600');
    });
  }
});
