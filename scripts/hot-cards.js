document.addEventListener('DOMContentLoaded', () => {
  ...
  dbRef.once('value').then(snap => {
    ...
    selected.forEach((card) => {
      ...
      cardEl.className = 'relative flex-none w-48 md:w-full snap-start rounded-xl bg-white shadow-lg transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl border-2 overflow-hidden';
      cardEl.style.borderColor = color;
      cardEl.innerHTML = `
        <div class="flex items-center justify-center h-56 sm:h-64 bg-white">
          <img src="${card.image}" alt="${displayName}" class="max-h-full max-w-full object-contain">
        </div>
