...
document.addEventListener("DOMContentLoaded", () => {
    ...
    waitForElement("#username-display", () => {
        ...
        const attachBalanceEffect = (el) => {
            ...
            const observer = new MutationObserver(() => {
                ...
                if (current !== prev) {
                    ...
                    el.addEventListener("animationend", () => el.classList.remove("balance-change"), { once: true });
                }
            });
        ...
        const dismissedNotifications = new Set(JSON.parse(localStorage.getItem("pullz-dismissed-notifications") || "[]").filter(Boolean));
        ...
        const attachNotificationStream = () => {
            if (unsubscribeNotifications || !notificationList)
                return;
        ...
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                handler = (snap) => {
                    ...
                    if (balanceMobile)
                        balanceMobile.innerText = balanceFormatted;
                    if (balanceDropdown)
                        balanceDropdown.innerText = balanceFormatted;
                    if (popupBalance)
                        popupBalance.innerText = `${balanceFormatted} gems`;
                };
                ...
                if (toolbarDesktop)
                    toolbarDesktop.classList.remove("hidden");
                if (balanceContainer)
                    balanceContainer.classList.remove("hidden");
                if (balanceMobileHeader)
                    balanceMobileHeader.classList.remove("hidden");
                if (toolbarMobile)
                    toolbarMobile.classList.remove("hidden");
                if (balanceMobileDrawer)
                    balanceMobileDrawer.classList.remove("hidden");
                if (topupBtn)
                    topupBtn.classList.remove("hidden");
                if (topupMobileHeader)
                    topupMobileHeader.classList.remove("hidden");
                if (topupMobileDrawer)
                    topupMobileDrawer.classList.remove("hidden");
                if (notificationBell)
                    notificationBell.classList.remove("hidden");
                if (notificationBellMobile)
                    notificationBellMobile.classList.remove("hidden");
                attachNotificationStream();
                if (themeToggleStandaloneDesktop)
                    themeToggleStandaloneDesktop.classList.add("hidden");
                if (themeToggleChipDesktop)
                    themeToggleChipDesktop.classList.remove("hidden");
                ...
                if (balanceDropdown)
                    balanceDropdown.innerText = "0";
                if (popupBalance)
                    popupBalance.innerText = "0 gems";
                if (authButtons)
                ...
                if (toolbarDesktop)
                    toolbarDesktop.classList.add("hidden");
                if (balanceContainer)
                    balanceContainer.classList.add("hidden");
                if (balanceMobileHeader)
                    balanceMobileHeader.classList.add("hidden");
                if (toolbarMobile)
                    toolbarMobile.classList.remove("hidden");
                if (balanceMobileDrawer)
                    balanceMobileDrawer.classList.add("hidden");
                if (topupBtn)
                    topupBtn.classList.add("hidden");
                if (topupMobileHeader)
                    topupMobileHeader.classList.add("hidden");
                if (topupMobileDrawer)
                    topupMobileDrawer.classList.add("hidden");
                if (notificationBell)
                    notificationBell.classList.add("hidden");
                if (notificationBellMobile)
                    notificationBellMobile.classList.add("hidden");
                hideNotificationPanel();
                ...
                if (themeToggleStandaloneDesktop)
                    themeToggleStandaloneDesktop.classList.remove("hidden");
                if (themeToggleChipDesktop)
                    themeToggleChipDesktop.classList.add("hidden");
                if (mobileRegisterBtn)
    ...
    waitForElement("#topup-button", () => {
        waitForElement("#topup-popup", () => {
            ...
            if (topupDesktop)
                topupDesktop.addEventListener("click", openTopup);
            if (topupMobileHeader)
                topupMobileHeader.addEventListener("click", openTopup);
            if (topupMobileDrawer)
                topupMobileDrawer.addEventListener("click", openTopup);
        });
    });
    // Dropdown toggle
    waitForElement("#dropdown-toggle", () => {
        ...
        const closeDropdown = () => {
            ...
            userDropdown.addEventListener("animationend", () => {
    ...
    waitForElement("#menu-toggle", () => {
        ...
        const closeMenu = () => {
            ...
            mobileDropdown.addEventListener("animationend", () => {
